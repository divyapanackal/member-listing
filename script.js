  ...
  .then((data) => {
      ...
      if (target.classList.contains("btn-edit")) {
        if (target.textContent === "Edit") {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            switch (i) {
              case 0:
                cells[
                  i
                ].innerHTML = `<input class="border border-indigo-600 p-2" type="text" value="${text}" />`;
                break;
              case 1:
                cells[
                  i
                ].innerHTML = `<input class="border border-indigo-600 p-2" type="email" value="${text}" />`;
                break;
              case 2:
                cells[
                  i
                ].innerHTML = `<input class="border border-indigo-600 p-2" tel="number" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" value="${text}" />`;
                break;
              case 3:
                cells[
                  i
                ].innerHTML = `<input class="border border-indigo-600 p-2" type="number" value="${text}" min="1" max="90" />`;
                break;
              case 4:
                cells[i].innerHTML = `<input type="radio" id="male-${
                  row.i
                }" name="gender-${row.i}" value="Male" ${
                  text === "Male" ? "checked" : ""
                }>
                                         <label for="male-${
                                           row.i
                  ...
                  row.i
                }" value="Female" ${text === "Female" ? "checked" : ""}>
                                        <label for="female-${
                                          row.i
                                        }">Female</label>`;
                break;
          ...
          cells.forEach((cell, i) => {
            ...
            if (i == 4) {
              const checkedRadio = cell.querySelector(
    ...
    form.addEventListener("submit", function (e) {
      ...
      function addCell(value) {
        ...
        cell.classList.add(
          ..."border-b border-gray-100 p-4 pl-8 text-gray-500 dark:border-gray-700 dark:text-gray-400".split(
            " "
          )
        );
        ...
        if (value === "actions") {
          cell.innerHTML =
            '<button class="me-2 btn-edit text-blue-600/100 cursor-pointer">Edit</button><button class="btn-delete text-red-500 cursor-pointer">Delete</button>';
        } else {
